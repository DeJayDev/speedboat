...
class TagsPlugin(Plugin):
    ...
    def on_tags_create(self, event, name, content):
        ...
        _, created = Tag.get_or_create(guild_id=event.guild.id, author_id=event.author.id, name=name, content=content)

        if not created:
            raise CommandFail("A tag by that name already exists")

        raise CommandSuccess("Ok, created a tag named `{}`".format(name))
    ...
    def on_tags_list(self, event):
        ...
        if not tags:
            raise CommandFail("No tags exist for this server")
    ...
    def on_tags_remove(self, event, name):
        ...
        if tag.author_id != event.author.id:
            if event.user_level <= event.config.min_level_remove_others:
                raise CommandFail("You do not have the permission to remove other users tags")
    ...
    def on_tags_info(self, event, name):
        try:
            tag = Tag.select(Tag, User).join(User, on=(User.user_id == Tag.author_id).alias("author")).where((Tag.guild_id == event.guild.id) & (Tag.name == S(name))).get()
        except Tag.DoesNotExist:
            raise CommandFail("No tag by that name exists")
